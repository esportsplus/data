...
// Domain agreement for non-encodable values: on the SAME codec instance, computeSize
// must throw the exact Codec2 error encode throws — captured live, never hardcoded, so
// the assertion tracks whichever path (unrepresentable / unencodable) the value hits.
function expectSameThrow(value: unknown): void {
    let codecs = [c, cc];

    for (let i = 0, n = codecs.length; i < n; i++) {
        let instance = codecs[i]!,
            encMsg: string | undefined;
...
function expectSameThrowAll(values: unknown[]): void {
    for (let i = 0, n = values.length; i < n; i++) {
        expectSameThrow(values[i]);
    }
}
...
describe('Codec2 computeSize — exact encoded byte length', () => {
    ...
    describe('clause 1 — measured divergences become exact', () => {
        ...
        it('a plain array: computeSize equals encoded length (was -1 vs 8)', () => {
            ...
            expect(c.encode(value).length).toBe(9);
            expect(c.computeSize(value)).toBe(c.encode(value).length);
        });

        it('a Map: computeSize throws exactly as encode does (was -1 vs a bogus 13)', () => {
            expectSameThrow(new Map<string, unknown>([['a', 1]]));
        });
    });
    ...
    describe('clause 2 — corpus holds computeSize === encoded length', () => {
        ...
        it('Map values throw domain-agreement errors', () => {
            expectSameThrowAll([
                new Map(),
        ...
        it('Set values throw domain-agreement errors', () => {
            expectSameThrowAll([
                new Set(),
                new Set([1, 2, 3]),
                new Set(['a', 'b']),
