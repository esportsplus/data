// Runtime helper code to be inlined into generated encode/decode functions
...
function _readString(buffer, offset) {
    ...
    let str = _textDecoder.decode(buffer.subarray(newOffset, newOffset + length));
...
const buildRuntimeHelpers = (needs: RuntimeNeeds): string => {
    ...
    // Varint must come first (string depends on it)
    if (needs.varint) {
